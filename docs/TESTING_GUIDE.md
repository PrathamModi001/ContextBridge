...
3. In devA terminal, type any task:
   ```
   add a comment to loginUser
   ```
4. Watch dashboard → a new node for `loginUser` should appear with devA's color
...
#### 5a — Accept A
Click **Accept A** button in merge editor.
- REPL terminals both print `[✓ RESOLVED]`
- devA's version written to both workspaces
- Dashboard conflict closes

#### 5b — Accept B
Trigger a new conflict. Click **Accept B**.
- devB's version written to both workspaces

#### 5c — Manual Merge
Trigger a new conflict. Edit the center panel in the merge editor. Click **Submit Merge**.
...
# Who uses loginUser (by devId)
curl http://localhost:3000/v1/entities/loginUser/clients
...
# if loginUser requires (email, password) → returns conflict with correctedCall
```
...
# devA> add rate limiting to validateLimit
# devB> add caching to validateLimit   (triggers conflict)
